...
export function FavoritesTab({ selectedInstance }: FavoritesTabProps) {
  ...
  useEffect(() => { loadAllFavoriteProjects() }, [favoriteMods, favoriteResourcePacks, favoriteShaderPacks])
  useEffect(() => { if (selectedInstance) loadInstalledFiles() }, [selectedInstance])

  const loadFavorites = () => {
    const parse = (key: string, setter: (s: Set<string>) => void) => {
      const saved = localStorage.getItem(key)
      if (saved) {
        try { setter(new Set(JSON.parse(saved))) } catch { console.error(`Failed to parse ${key}`) }
      }
    }
    parse('favoriteMods', setFavoriteMods)
    parse('favoriteResourcePacks', setFavoriteResourcePacks)
    parse('favoriteShaderPacks', setFavoriteShaderPacks)
  }
  ...
  const loadInstalledFiles = async () => {
    ...
    try {
      const [mods, resourcepacks, shaderpacks] = await Promise.all([
        invoke<{ filename: string }[]>("get_installed_mods", { instanceName: selectedInstance.name }).catch(() => []),
        invoke<string[]>("get_installed_resourcepacks", { instanceName: selectedInstance.name }).catch(() => []),
        invoke<string[]>("get_installed_shaderpacks", { instanceName: selectedInstance.name }).catch(() => []),
      ])
  ...
  const loadAllFavoriteProjects = async () => {
    ...
    if (allFavoriteIds.size === 0) { setFavoriteProjects([]); return }
  ...
  const handleInstallAllFavorites = async () => {
    ...
    for (let i = 0; i < favoriteProjects.length; i++) {
      ...
      try {
        ...
        if (versions.length === 0) { setInstallProgress(prev => ({ ...prev, current: i + 1, failed: prev.failed + 1 })); continue }

        const latestVersion = versions[0]
        const primaryFile = latestVersion.files.find(f => f.primary) || latestVersion.files[0]
        if (!primaryFile) { setInstallProgress(prev => ({ ...prev, current: i + 1, failed: prev.failed + 1 })); continue }
        if (installedFiles.has(primaryFile.filename)) { setInstallProgress(prev => ({ ...prev, current: i + 1 })); continue }

        if (project.project_type === "mod") {
          await invoke<string>("download_mod", { instanceName: selectedInstance.name, downloadUrl: primaryFile.url, filename: primaryFile.filename })
        } else if (project.project_type === "resourcepack") {
          await invoke<string>("download_resourcepack", { instanceName: selectedInstance.name, downloadUrl: primaryFile.url, filename: primaryFile.filename })
        } else if (project.project_type === "shader") {
          await invoke<string>("download_shaderpack", { instanceName: selectedInstance.name, downloadUrl: primaryFile.url, filename: primaryFile.filename })
        }
    ...
    <div className="max-w-7xl mx-auto">
      ...
      {totalFavorites === 0 ? (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-[#e6e6e6] mb-2">No favorites yet</h3>
          <p className="text-sm text-[#7d8590] mb-1">Start adding your favorite mods, resource packs, and shader packs</p>
          <p className="text-xs text-[#3a3f4b]">Click the heart icon on any item to add it to your favorites</p>
        </div>
      ) : (
        <>
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 space-y-3">
                {favoriteProjects.map((project) => (
                  <div key={project.project_id} className="blur-border rounded-md overflow-hidden bg-[#22252b]">
                    <div className="flex min-h-0 relative z-0">
                      {project.icon_url ? (
                        <div className="w-24 h-24 flex items-center justify-center flex-shrink-0 rounded m-2">
                          <img src={project.icon_url} alt={project.title} className="w-full h-full object-contain rounded" />
                        </div>
                      ) : (
              ...
              <div className="bg-[#22252b] rounded-md p-5 sticky top-4 self-start border border-[#3a3f4b]">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-semibold text-[#e6e6e6] mb-2">Install All Favorites</h3>
                  <p className="text-sm text-[#7d8590] mb-6">Quickly install all your favorited items to the selected instance</p>
                  <button
                  ...
                  >
                    {isInstallingAll ? (
                      <><Loader2 size={16} className="animate-spin" />{"Installing..."} ({installProgress.current}/{installProgress.total})</>
                    ) : (
                      <><Download size={16} />{"Install All"} ({favoriteProjects.length})</>
                    )}
                  </button>
                  {installProgress.failed > 0 && (
                    <p className="text-xs text-[#ef4444] mt-2">Failed to install {installProgress.failed} items</p>
                  )}
                  {!selectedInstance && favoriteProjects.length > 0 && (
                    <p className="text-xs text-[#7d8590] mt-3">Select an instance to install favorites</p>
                  )}
                </div>
              </div>
            </div>
