...
interface CurseforgeResourcePacksTabProps {
...
export function CurseforgeResourcePacksTab({ selectedInstance, hideToolbar, sourceSelector, modsSelector, searchQuery, onSearchQueryChange }: CurseforgeResourcePacksTabProps) {
              ...
              <div className="flex min-h-0 relative z-0">
                {item.logo?.thumbnailUrl ? (
                  ...
                  <div className="w-24 h-24 bg-gradient-to-br from-[#8b5cf6]/10 to-[#a78bfa]/10 flex items-center justify-center flex-shrink-0 rounded m-2">
                    <Package size={48} className="text-[#8b5cf6]" />
                  </div>
                )}
        ...
        {selectedItem && (
          <div className="bg-[var(--bg-tertiary)] rounded-md p-3 sticky top-0 self-start">
            ...
            <div className="pt-1">
              ...
              {isLoadingFiles ? (
                <div className="text-center py-6"><Loader2 size={20} className="animate-spin text-[#8b5cf6] mx-auto" /></div>
              ) : itemFiles.length === 0 ? (
