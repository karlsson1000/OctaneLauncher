...
import { Search, Download, Loader2, Package, CheckCircle, AlertCircle } from "lucide-react"
...
export function CurseforgeModpacksTab({ instances, hideToolbar, sourceSelector, modsSelector, searchQuery, onSearchQueryChange, onShowCreationToast, onRefreshInstances }: CurseforgeModpacksTabProps) {
  ...
  const [installingFiles, setInstallingFiles] = useState<Set<number>>(new Set())
  ...
  const handleInstall = async (file: CurseforgeFile, modpack: CurseforgeHit) => {
    const downloadUrl = file.downloadUrl
    if (!downloadUrl) return

    setInstallingFiles(prev => new Set(prev).add(file.id))
    try {
      ...
      setInstallationStatus(prev => ({ ...prev, [modpack.id.toString()]: "success" }))
      if (onRefreshInstances) setTimeout(() => onRefreshInstances!(), 500)

      setTimeout(() => {
        setInstallingFiles(prev => { const s = new Set(prev); s.delete(file.id); return s })
        setInstallationStatus(prev => { const s = { ...prev }; delete s[modpack.id.toString()]; return s })
      }, 3000)
    } catch (error) {
      console.error("Failed to install modpack:", error)
      setInstallationStatus(prev => ({ ...prev, [modpack.id.toString()]: "error" }))
      setInstallingFiles(prev => { const s = new Set(prev); s.delete(file.id); return s })
      setTimeout(() => {
        setInstallationStatus(prev => { const s = { ...prev }; delete s[modpack.id.toString()]; return s })
      }, 5000)
                  ...
                  {itemFiles.map((file) => {
                    ...
                    return (
                      <div key={file.id} className="bg-[var(--bg-secondary)] rounded p-3 flex items-center justify-between gap-2">
                        ...
                        <button
                          onClick={() => handleInstall(file, selectedItem)}
