      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-6 pt-6 pb-5">
            <h2 className="text-xl font-semibold text-[var(--text-primary)] tracking-tight">New Instance</h2>
          ...
          <div className="px-6 pb-4 space-y-5">
            <div className="flex">
              <button type="button" onClick={handleImportFile} disabled={isCreating} className="w-full px-4 py-3 rounded text-sm font-medium transition-all cursor-pointer bg-[var(--bg-tertiary)] hover:bg-[var(--bg-hover-strong)] text-[var(--text-muted)]">
                <div className="flex items-center justify-center gap-2">
                  <FileDown size={18} className="text-[var(--text-muted)]" strokeWidth={2} />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full bg-[var(--bg-tertiary)] rounded px-4 py-3.5 text-sm text-[var(--text-primary)] placeholder-gray-500 focus:outline-none transition-all ${instanceExists && newInstanceName.trim() ? 'ring-2 ring-red-500' : ''}`}
                disabled={isCreating}
              />
            ...
            <div>
              <div className="flex items-center justify-between mb-2.5">
                <label className="text-sm font-medium text-[var(--text-primary)]">Minecraft Version</label>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => handleVersionFilterChange("release")}
                    disabled={isCreating}
                    className={`text-xs font-medium transition-colors cursor-pointer ${versionFilter === "release" ? "text-[var(--text-primary)]" : "text-[var(--text-muted)] hover:text-[var(--text-muted)]"}`}
                  >
                    Releases
                  </button>
                    ...
                    <div className="absolute z-10 w-full bg-[var(--bg-tertiary)] rounded-b shadow-lg max-h-60 overflow-y-auto custom-scrollbar border-t border-[var(--bg-elevated)]">
                      {filteredVersions.map((version) => (
                        ...
                        >
                          ...
                          {selectedVersion === version.id && <Check size={16} className="text-[var(--text-primary)]" strokeWidth={2} />}
                        </button>
                      ))}
                    </div>
                  )}
            ...
            {loaderType === "neoforge" && (
              <div>
                <label className="block text-sm font-medium text-[var(--text-primary)] mb-2.5">NeoForge Version</label>
                {isLoadingNeoforge ? (
                  <div className="flex items-center gap-2 text-[var(--text-muted)] text-sm py-3.5 px-4 bg-[var(--bg-tertiary)] rounded">
          ...
          <div className="flex items-center justify-end gap-3 px-6 pb-6 pt-3">
            <button onClick={handleClose} disabled={isCreating} className="px-5 py-3 bg-[var(--bg-tertiary)] hover:bg-[var(--bg-hover-strong)] text-[var(--text-primary)] rounded font-medium text-sm transition-colors disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed">
              Cancel
            </button>
            <button onClick={handleCreateInstance} disabled={isCreateDisabled} className="px-5 py-3 bg-[var(--accent-primary)] hover:bg-[var(--accent-hover)] text-white rounded font-medium text-sm flex items-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
              {isCreating ? (
