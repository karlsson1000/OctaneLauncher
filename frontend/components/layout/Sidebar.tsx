...
export function Sidebar(props: SidebarProps) {
  ...
  const handleSendFriendRequest = async () => {
    ...
    try {
      ...
      if (errorMsg.includes("Friend request already sent")) displayMsg = "Already sent friend request"
      else if (errorMsg.includes("already sent you a friend request")) displayMsg = "Check your friend requests!"
      else if (errorMsg.includes("User") && errorMsg.includes("not found")) displayMsg = "User not found"
      else if (errorMsg.includes("Already friends")) displayMsg = "Already friends"
  ...
  const handleFriendClick = async (_friend: Friend) => {
    // no-op
  }
      ...
      {sidebarContextMenu && (
        <ContextMenu
          ...
          items={[
            ...
            { label: "Open Folder", icon: <FolderOpen size={16} />, onClick: () => onOpenInstanceFolder(sidebarContextMenu.instance) },
            { label: "Duplicate", icon: <Copy size={16} />, onClick: () => onDuplicateInstance(sidebarContextMenu.instance) },
            { separator: true },
            { label: "Delete", icon: <Trash2 size={16} />, onClick: () => onDeleteInstance(sidebarContextMenu.instance.name), danger: true },
          ]}
        />
      )}
