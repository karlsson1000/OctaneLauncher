...
import type { Instance, FabricVersion, LauncherSettings } from "../../types"
...
function InnerModal({
  ...
  const handleSaveRam = async (memoryMb: number, enabled: boolean) => {
    try {
      ...
      setAlertModal({ isOpen: true, title: t('common.errors.title'), message: `Failed to save RAM settings: ${String(error)}`, type: "danger" })
    }
  }
  ...
  const loadMinecraftVersions = async () => {
    ...
    try {
      ...
      setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.loadVersionsFailed', { error: String(error) }), type: "danger" })
    } finally {
  ...
  const loadFabricVersions = async () => {
    ...
    try {
      ...
      setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.loadFabricVersionsFailed', { error: String(error) }), type: "danger" })
    } finally {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.updateVersionFailed', { error: String(error) }), type: "danger" })
  ...
  const handleFabricSelect = async (newVersion: string) => {
    ...
    try {
      ...
      setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.updateFabricFailed', { error: String(error) }), type: "danger" })
  ...
  const handleIconChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setAlertModal({ isOpen: true, title: t('errors.invalidFile'), message: t('instanceSettings.errors.invalidImageFile'), type: "danger" })
      return
    }

    if (file.size > 5 * 1024 * 1024) {
      setAlertModal({ isOpen: true, title: t('errors.fileTooLarge'), message: t('instanceSettings.errors.imageTooLarge'), type: "danger" })
      return
    }
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          ...
          setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.setIconFailed', { error: String(error) }), type: "danger" })
        } finally {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.removeIconFailed', { error: String(error) }), type: "danger" })
        }
      }
    })
  }
  ...
  const handleRename = async (trimmedName: string) => {
    if (!trimmedName) { setRenameError(t('instanceSettings.errors.emptyName')); return }
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setAlertModal({ isOpen: true, title: t('errors.generic'), message: t('instanceSettings.errors.deleteInstanceFailed', { error: String(error) }), type: "danger" })
        } finally {
          setIsDeleting(false)
        }
      }
          ...
          <div className="px-6 pb-6">
            <div className="grid grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div className="col-span-2 pt-3 border-t border-[#3a3f4b]">
                ...
                >
                  {isDeleting
                    ? <><Loader2 size={16} className="animate-spin" /><span>{t('instanceSettings.deleteInstance.deleting')}</span></>
                    : <><Trash2 size={16} /><span>{t('instanceSettings.deleteInstance.button')}</span></>
                  }
                </button>
              </div>
            </div>
