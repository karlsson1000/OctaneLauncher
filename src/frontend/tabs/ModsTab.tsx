...
export function ModsTab({ selectedInstance, instances, onSetSelectedInstance }: ModsTabProps) {
  ...
  // Auto-select first Fabric instance if current selection is not Fabric
  useEffect(() => {
  ...
  // Load installed mods whenever the selected instance changes
  useEffect(() => {
    if (selectedInstance && selectedInstance.loader === "fabric") {
      loadInstalledMods()
    }
  ...
  const loadInstalledMods = async () => {
    ...
    try {
      ...
      // Create a set of installed filenames for quick lookup
  ...
  const isModInstalled = (version: ModrinthVersion): boolean => {
    // Check if any of the version's files are installed
  ...
  const handleDownloadMod = async (version: ModrinthVersion) => {
    ...
    try {
      ...
      // Add the filename to installed mods
    ...
    <div className="p-6 space-y-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-semibold text-[#e8e8e8] tracking-tight">Mods</h1>
            <p className="text-sm text-[#808080] mt-0.5">Browse and install mods from Modrinth</p>
          </div>
          {selectedInstance && selectedInstance.loader === "fabric" && (
