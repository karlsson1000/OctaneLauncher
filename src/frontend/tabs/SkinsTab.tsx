...
export function SkinsTab(props: SkinsTabProps) {
  ...
  const handleCapeSelect = async (capeId: string) => {
    ...
    } catch (err) { setError(`${t('skins.errors.equipCapeFailed')}: ${err}`) }
  }
  ...
  const handleCapeRemove = async () => {
    ...
    try {
      ...
      setActiveCape(null)
      handleCloseCapeModal()
    } catch (err) { setError(`${t('skins.errors.removeCapeFailed')}: ${err}`) }
  }
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        setError(null)
      } catch (err) { setError(`${t('skins.errors.uploadFailed')}: ${err}`) } finally { setUploading(false) }
    }
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setTimeout(() => loadUserSkin(false), 2000)
    } catch (err) { setError(`${t('skins.errors.resetFailed')}: ${err}`) } finally { setResetting(false) }
  }
        ...
        <div className="flex gap-24 items-center justify-center">
          <div className="flex-shrink-0 self-start mt-8">
            <div className="flex flex-col items-center relative">
              <button onClick={() => setShowBack(prev => !prev)} className="absolute top-0 -right-8 z-10 p-2 bg-[#22252b] hover:bg-[#2a2d35] text-[#e6e6e6] rounded-md transition-all cursor-pointer" title={t('skins.rotateTooltip')}>
                <Rotate3d size={20} />
              </button>
              <div className="rounded-md overflow-hidden bg-[#181a1f] p-4">
                ...
                {!loading && currentSkinHash && (
                  <img src={getSkinRenderUrl() || ''} alt={t('skins.skinRenderAlt')} className="w-[250px] h-[406px]" style={{ imageRendering: 'pixelated' }} />
                )}
                {!loading && !currentSkinHash && (
          ...
          <div className="flex-1 max-w-sm space-y-4">
            <div className="blur-border bg-[#22252b] rounded-md p-5">
              ...
              <div className="flex gap-2 mb-6">
                <button onClick={() => setSkinVariant("classic")} className={`flex-1 px-4 py-2.5 rounded-md text-sm font-medium transition-all cursor-pointer ${skinVariant === "classic" ? "bg-[#4572e3] text-white" : "bg-[#181a1f] text-[#7d8590] hover:bg-[#1f2128] hover:text-[#e6e6e6]"}`}>
                  {t('skins.variants.classic')}
                </button>
                <button onClick={() => setSkinVariant("slim")} className={`flex-1 px-4 py-2.5 rounded-md text-sm font-medium transition-all cursor-pointer ${skinVariant === "slim" ? "bg-[#4572e3] text-white" : "bg-[#181a1f] text-[#7d8590] hover:bg-[#1f2128] hover:text-[#e6e6e6]"}`}>
              ...
              <div className="blur-border bg-[#22252b] rounded-md p-5">
                ...
                <div className="flex gap-3">
                  {recentSkins.map((skin, index) => {
                    ...
                    return (
                      <button key={`${skin.url}-${index}`} onClick={() => handleRecentSkinSelect(skin)} disabled={uploading} className="bg-[#181a1f] rounded-md hover:ring-2 hover:ring-[#4572e3] transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer overflow-hidden" title={t('skins.recentSkinTooltip')}>
                        <img src={renderUrl} alt={t('skins.recentSkinAlt')} className="w-20 h-20" style={{ imageRendering: 'pixelated' }} />
                      </button>
                    )
                  })}
                </div>
          ...
          <div className={`blur-border bg-[#181a1f] rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto modal-content ${isClosingModal ? 'closing' : ''}`} onClick={(e) => e.stopPropagation()}>
            ...
            {loadingCapes ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-5 gap-3">
                  {capes.map((cape) => (
                    <button
                      ...
                      className={`aspect-[5/8] bg-[#181a1f] rounded-md overflow-hidden flex flex-col items-center justify-center transition-all cursor-pointer hover:ring-2 hover:ring-[#4572e3] ${activeCape === cape.id ? "ring-2 ring-[#4572e3]" : ""}`}
                      title={cape.alias}
                    >
                ...
                {activeCape && (
                  <button onClick={handleCapeRemove} className="w-full px-4 py-3 bg-[#22252b] hover:bg-[#2a2d35] text-[#e6e6e6] rounded-md text-sm font-medium transition-all cursor-pointer">
                    {t('skins.removeCape')}
                  </button>
                )}
              </div>
