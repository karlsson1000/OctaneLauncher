            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-[#22252b] rounded-md p-5 sticky top-4 self-start border border-[#3a3f4b]">
                <div className="text-center">
                  ...
                  >
                    {isInstallingAll ? (
                      <><Loader2 size={16} className="animate-spin" />{t('favorites.installAll.installing')} ({installProgress.current}/{installProgress.total})</>
                    ) : (
                      <><Download size={16} />{t('favorites.installAll.button')} ({favoriteProjects.length})</>
                    )}
                  </button>
                  {installProgress.failed > 0 && (
                    <p className="text-xs text-[#ef4444] mt-2">{t('favorites.installAll.failed', { count: installProgress.failed })}</p>
                  )}
                  {!selectedInstance && favoriteProjects.length > 0 && (
                    <p className="text-xs text-[#7d8590] mt-3">{t('favorites.installAll.selectInstance')}</p>
                  )}
                </div>
              </div>
            </div>
